...
from datasurface.md.Governance import DataContainer, DataPlatform, DataTransformerOutput, DatasetGroup, DatasetSink, Datastore, Ecosystem, IngestionConsistencyType, StepTrigger, Workspace
    ...
    def generateGraph(self):
        ...
        for dsg in self.roots:
            if dsg.workspace.dataContainer:
                ...
                if self.dataContainerExports.get(dataContainer) == None:
                    ...
                    for dsgR in self.roots:
                        if dsgR.workspace == w:
                            for sink in dsgR.dsg.sinks.values():
                                dsrExportStep : ExportNode = cast(ExportNode, self.findExistingOrCreateStep(ExportNode(self.platform, w.dataContainer, sink.storeName, sink.datasetName)))
