...
def ddlColumnToSQLAlchemyType(dataType : DDLColumn) -> sqlalchemy.Column[Any]:
    ...
    c : sqlalchemy.Column[Any] = sqlalchemy.Column(dataType.name, t, primary_key=(dataType.primaryKey == PrimaryKeyStatus.PK), nullable=(dataType.nullable == NullableStatus.NULLABLE))
    return c
...
def convertSQLAlchemyTableToDataset(table : sqlalchemy.Table) -> Dataset:
    ...
    for constraint in table.constraints:
        if(type(constraint) == sqlalchemy.PrimaryKeyConstraint):
