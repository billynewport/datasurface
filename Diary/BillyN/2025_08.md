...
The airflow image worked fine. I'm now working on AWS as a first cloud version of Yellow. Aurora or AWS RDS are postgres compatible so thats already done. AWS EKS/Fargate is kubernetes compatible so thats ok. AWS EFS will be the storage class for the git clone cache. The secret management is the main change needed. I am planning on having all secrets managed by AWS secrets manager. They will be hierarchically organized like this:

* datasurface/merge/rw_credential
* datasurface/git
* datasurface/zone/team/sourceName

There will a service accounts for Airflow and all pods, so 2 service accounts in total, airflow/ingestion. The airflow service account will have access to all secrets in datasurface/**. The ingestion service account has access to nothing. Pods will be provided the secrets they need as environment variables populated by the airflow DAG when its launched. The main work with getting this done is to create a new airflowAWS component and a new airflowAWS template folder for it also. The way infrastructure_dag.py is generated needs to be refactored and delegated to the Assembly where each component will generate artifacts for its component. The assembly class will get a generateBootstrapArtifacts method which will in turn call the generateBootstrapArtifacts method for each component. This will let each component make a number of files, one for the airflow DAG, a terraform file, an AWS CDK file and so on.

Before starting, these are the changes that I need, I guess we'll see how it goes...

### Masked pylance issues

I noticed an issue where pylance did not flag an Optional type as a "might be none" errror. This was because the AI assistant had configured pylance previously to ignore these as well as ignore overridden method issues. I fixed the masking issue and ended up with 200 errors in the code. The code works but pylance didn't like it. I went through all the issues and fixed them. This is the reason for the huge commit today.

I'm still working through the AWS version and the refactoring in Yellow to allow that. All the local tests are passing for now.

## 2025/08/31

AWS work starting. I have created an account and a VPC. I started a small EC2 node with tailscale running on it so I can VPN to resources there as if on my local network. I have a micro postgres database running on a seperate instances, so 3 instances in total, tailscale, workarea, postgres.

Review for tomorrows work.

* Define an EFS instance
* Need to define an efs-sc storage class for the git cache
* psp_airflow.yaml.j2 needs to be updated to use the a PVC for the git cache, it's currently emptyDir.
